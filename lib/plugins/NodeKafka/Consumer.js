...
class KafkaConsumer {
  ...
  connectToKafka(options, done) {
    let self = this;
    self.client = new kafka.Client(options.connectionString, options.clientId, options.zkOptions, options.noAckBatchOptions);
    var topics = [
      {topic: self.topic},
    ];
    var consumerOptions = {
    ...
    self.consumer = new Consumer(self.client, topics, _.merge(consumerOptions, self.kafkaConsumerOptions));
  ...
  get stream() {
    if (this[_stream] == null) {
      this[_stream] = this.rawStream(through2.obj(function(data, enc, cb) {
        cb(null, data.value);
      }));
    }
    return this[_stream];

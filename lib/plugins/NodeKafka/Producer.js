...
class KafkaProducer {
   ...
   * @param {Object} options.streamOptions - An object with stream options to
   *    be passed to the transform stream constructor.
   * @param {Mixed} options.kafkaClientOptions - Options to be passed to
   *    kafka-node's Client() constructor.
   * @param {Mixed} options.version - Version number to be included with message
   *    envelope, coerced to a string. Defaults to empty string.
   * @param {Function} done - A function to call after the producer has been
  ...
  constructor(options, done) {
    ...
    this[_stream] = through2.obj(streamOptions, this.streamProcessor.bind(this));
    var self = this;
